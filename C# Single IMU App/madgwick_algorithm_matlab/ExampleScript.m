...
AHRS = MadgwickAHRS('SamplePeriod', 1/125, 'Beta', 0.1);
% AHRS = MahonyAHRS('SamplePeriod', 1/256, 'Kp', 0.5);

quaternion = zeros(length(time), 4);
for t = 1:length(time)
    AHRS.UpdateIMU(Gyroscope(t,:) * (pi/180), ...
        Accelerometer(t,:));%, Magnetometer(t,:));	
...
%%
save('data.mat','time','Accelerometer','Gyroscope','euler');